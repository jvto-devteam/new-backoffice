...
class PackageController extends Controller
{
    ...
    {
        $orderChannel = 'jvto';
        $data['packages'] = Package::select('id', 'name', 'short_description', 'duration_id', 'category_id', 'start_destination_id', 'end_destination_id', 'id_url', 'url', 'package_code')->with([
        ...
        ])->where('is_publish', '1')->where('category_id', 1)->get();
        // return $data['packages'];
        $data['packages'] = $data['packages']->map(function ($package) {
            ...
            return [
                'id' => $package->id,
                'name' => $package->name,
                'category' => "Private Package",
                'short_description' => $package->short_description,
                'duration' => ($day == 1 ? $day . ' Day' : $day . ' Days') . ' ' . ($night == 1 ? $night . ' Night' : $night . ' Nights'),
                'package_start_from' => $package->startDestination->name,
                'package_end_at' => $package->endDestination->name,
                'price_start' => $package->min_price,
                'url_package' => $url,
                'destinations' => $package->packageDestination->pluck('destination.name')->toArray(),
                'itineraries' => $package->itinerary->sortBy('day')->map(function ($itinerary) use ($package) {
                    $meals = [];
                    if (!empty($itinerary->itineraryMeals[0])) {
                        if ($itinerary->itineraryMeals[0]->breakfast) {
                            array_push($meals, 'Breakfast');
                        }
                        if ($itinerary->itineraryMeals[0]->lunch) {
                            array_push($meals, 'Lunch');
                        }
                        if ($itinerary->itineraryMeals[0]->dinner) {
                    ...
                    return [
                        ...
                        'accommodation' => [
                            'hotel' => $package->packageHotel->where('day', $itinerary->day)->first() ? $package->packageHotel->where('day', $itinerary->day)->first()->hotel->name : null,
                            'room_hotel_configuration' => $package->packageHotel->where('day', $itinerary->day)->first() ? $package->packageHotel->where('day', $itinerary->day)->first()->hotel->roomHotelConfiguration->map(function ($roomConfig) {
                                return [
                                    'numb_of_pax' => $roomConfig->pax,
                                    'room_name' => $roomConfig->room->room_name,
                                    'room_qty' => $roomConfig->qty . ' Room(s)',
                                ];
                            })->toArray() : [],
                        ],
                ...
                'price_per_pax' => $package->packagePrice->map(function ($price) {
                    return [
                        'pax' => $price->priceCategory->temp_text,
                        'price' => number_format($price->price, 0, ',', '.'),
                    ];
                })->toArray(),
                'included' => [
                    'Private air-conditioned Transportation.',
                    'In-person Tour guide is licensed and certified.',
                    'All Accommodations as mentioned in the itinerary.',
                    'All permits and entry fees as mentioned in the itinerary.',
                    'Equipment: Gas Mask, Trekking Pole.',
                    'Travel T-Shirt.',
