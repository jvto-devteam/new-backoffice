                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div className="space-y-2">
                            ...
                            <SearchableSelect
                                ...
                                onChange={(value) =>
                                    handleAdditionalInfoChange("station", value)
                                }
...
const AddBooking = ({
    ...
    const handleAddNewAddOn = (newAddOn) => {
        console.log('Adding new add-on:', newAddOn); // Debug log
        setNewAddOns(prev => {
            const updated = [...prev, newAddOn];
            console.log('Updated newAddOns:', updated); // Debug log
            return updated;
        });
    };    
    // Gabungkan add-ons yang sudah ada dengan yang baru
    const allAddOns = [
        ...addOns.map(addon => ({
            id: addon.value,
            name: addon.label,
            ...
            <div className="flex gap-4 items-start">
                <div className="flex-1">
                    <SearchableSelect
                        ...
                        onChange={(value) =>
                            onChange(dayIndex, roomIndex, "room", value)
                        }
                        placeholder="Select room type"
                        open={roomSelection.isOpen} // Tambahkan state untuk setiap room selection
                        setOpen={(value) =>
                            onChange(dayIndex, roomIndex, "isOpen", value)
                        }
                        displayKey="name"
                    />
                </div>
    ...
    const calculateSummary = () => {
        ...
        if (channel === "TWT") {
            ...
            const totalAddOn = addOnItems.reduce(
        ...
        // Calculate total add-on
        const totalAddOn = addOnItems.reduce(
            (total, item) => parseInt(total) + parseInt(item.subtotal),
            0,
        );
        console.log('a');
        ...
        // Calculate grand total
        const subTotal = totalPackage + parseInt(totalAddOn);
    ...
    useEffect(() => {
        if (formData.packageName && !isCustomPackage) {
            const selectedPackage = packages.find(
                (pkg) => pkg.value === formData.packageName,
            );
            if (selectedPackage) {
    ...
    const renderStep3 = () => {
        const handlePackageSelection = (value) => {
            ...
            if (channel === "TWT") {
                ...
                const totalPrice =
                    selectedPackage.prices[0].pricePerPax *
                    (formData.numOfPax || 1);
                ...
                setFormData((prev) => ({
                    ...
                    packageDays: Array(selectedPackage.prices[0].start)
                        .fill()
                        .map(() => ({
                            startActivity: "",
                            endActivity: "",
                ...
                setFormData((prev) => {
                    ...
                    const mappedPackageDays = Array(packageDays)
                        .fill()
                        .map((_, index) => {
                            // Find itinerary data for current day
                            const dayItinerary =
                                selectedPackage.itineraries.find(
                                    (it) => it.day === index + 1,
                                );
                            // Find hotel data for current day
                            const dayHotel = selectedPackage.hotels.find(
                                (h) => h.day === index + 1,
                            );

                            return {
                                startActivity: dayItinerary
                                    ? dayItinerary.activity_start_id
                                    : "",
                                endActivity: dayItinerary
                                    ? dayItinerary.activity_end_id
                                    : "",
        ...
        const handleActivityChange = (dayIndex, field, value) => {
            setFormData((prev) => {
                ...
                const selectedActivity =
                    field === "startActivity"
                        ? startActivityOptions.find(
                              (activity) => activity.id === value,
                          )
                        : endActivityOptions.find(
                              (activity) => activity.id === value,
                          );
                // Update aktivitas
                ...
                if (field === "startActivity") {
                    ...
                    const startItinerary = selectedActivity
                        ? selectedActivity.itinerary
                        : "";
                    const endItinerary = endActivity
                        ? endActivityOptions.find(
                              (activity) => activity.id === endActivity,
                          )?.itinerary
                        : "";
                    ...
                    const endItinerary = selectedActivity
                        ? selectedActivity.itinerary
                        : "";
                    const startItinerary = startActivity
                        ? startActivityOptions.find(
                              (activity) => activity.id === startActivity,
                          )?.itinerary
                        : "";
            ...
            <div className="space-y-6 border rounded-lg">
                ...
                <div className="space-y-6 p-6 pt-0">
                                    ...
                                    onValueChange={(value) => {
                                        setFormData((prev) => ({
                                            ...
                                            pricePerPax: (
                                                (value || 0) /
                                                (prev.numOfPax || 1)
                                            ).toFixed(0), // Hitung price per pax
                                        }));
                                    }}
                                    className="w-full px-4 py-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                    ...
                    {(formData.packageName ||
                        formData.packageDays.length > 0) && (
                        <div className="mt-6 space-y-4">
                            {formData.packageDays.map((day, index) => (
                                <div
                                    key={index}
                                    className="border rounded-lg space-y-4"
                                >
                                    <div
                                                            ...
                                                            >
                                                                <div className="flex-1">
                                                                    <SearchableSelect
                                                                        ...
                                                                        disabled={
                                                                            !day.hotel
                                                                        } // Disable jika belum memilih hotel
                                                                        open={
                                                                            dropdowns[
                                                                                `day-${index}-room-${roomIndex}`
                                                                            ] ||
                                                                            false
                                                                        }
                                                                        setOpen={(
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="md:p-2 text-red-500 hover:text-red-700"
                                                                >
                                                                    <X className="h-5 w-5" />
                                                                </button>
                                                            </div>
...
const renderStep4 = () => {
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                        ...
                        <label
                            ...
                            className={`toggle-label block overflow-hidden h-6 rounded-full cursor-pointer ${
                                isCustomDiscount ? "bg-blue-500" : "bg-gray-300"
                            }`}
                        />
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <SearchableSelect
                                ...
                                setOpen={(value) =>
                                    handleDropdownChange(`discount-code`, value)
                                }
                            />
                        </div>
                        <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <SearchableSelect
                                ...
                                setOpen={(value) =>
                                    handleDropdownChange(`discount-type`, value)
                                }
                            />
                        </div>
                        <div className="space-y-2">
          ...
          `,
            ...
            if (result.isConfirmed) {
                const isWhatsappSelected =
                    channel != "TWT"
                        ? document.getElementById("whatsapp-itinerary").checked
                        : false;
    ...
    const addOnsInfo = addOnItems.map((item) => {
        let addOnId = item.addOn;
        let isNewAddOn = false;
        
        // Jika ini add-on baru, tambahkan informasi tambahan
        if (item.isNew || (typeof addOnId === 'string' && addOnId.startsWith('new-'))) {
            isNewAddOn = true;
            // Ambil detail add-on dari newAddOns array
            const tempIndex = parseInt(addOnId.replace('new-', ''));
            const newAddOnDetail = newAddOns[tempIndex];
        ...
        return {
            ...
            isNew: isNewAddOn
                ...
                const completeData = {
                    ...
                    ...packageInfo,
                    addOns: addOnsInfo,
                    newAddOns: newAddOns.map((addon, index) => ({
                        ...addon,
                        tempId: `new-${index}` // Tambahkan temporary ID untuk mapping
                    })),
                ...
                Object.keys(completeData).forEach((key) => {
                    if (typeof completeData[key] === "object") {
                        formDataObj.append(
                            key,
                            JSON.stringify(completeData[key]),
                        );
                    } else {
    ...
    return (
        <Authenticated>
            ...
            <h1 className="text-2xl mb-4 font-bold text-gray-900 dark:text-white">
                Add Booking {channel}
            </h1>
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-semibold">
                                            ...
                                            {new Intl.NumberFormat(
                                                ...
                                                calculateSummary().pricePerPax,
                                            )}
                                        </span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">
                                            Total Package ({formData.numOfPax}{" "}
                                            pax)
                                        </span>
                                        <span className="font-semibold">
                                            Rp{" "}
