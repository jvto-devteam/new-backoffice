...
class VendorController extends Controller
{
    function accommodation(Request $request, $id) {
        ...
        $bookHotel = [];
        $hotel = [];
        $tab = $request->tab ? $request->tab : 'overview';

        if($tab == 'schedule'){
            $bookHotel = BookHotel::with(['bookHotelMeal','booking.user', 'bookingItinerary', 'hotel', 'bookRoom.roomHotel']);
            if($request->bookingId){
                $bookHotel = $bookHotel->where('booking_id', $request->bookingId);
            }
            $bookHotel = $bookHotel->where('hotel_id', $id)
        ...
        ->with(['roomHotel' => function($query) {
            $query->with('roomPhoto')->select('hotel_id','id','room_name','rate');
        ...
        $accommodation = [
            'url_update' => config('app.core_url')."/api/new-backoffice/hotel/room-hotel/update", 
            'core_url' => config('app.core_url'), 
            'hotel' => $hotel,
            'isBookingId' => $request->bookingId,
            'schedule' => $bookHotel,
            'document' => [
                [
                    "id" => 1,
        ...
        return Inertia::render('Vendor/Accommodation', ['initialFilters' => $initialFilters,'hotelData' => $accommodation, 'tab' => $tab]);
    }
}
