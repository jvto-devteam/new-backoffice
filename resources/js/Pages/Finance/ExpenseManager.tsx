...
const FinanceDashboard = ({ booking = [], filters = {} }) => {
  ...
  const getFilteredData = () => {
    return booking.filter(row => {
      ...
      // Filter by month
      if (localFilters.month && row.start_date) {
        if (!row.start_date.startsWith(localFilters.month)) return false;
      }
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      paid: { class: 'bg-green-100 text-green-700', text: 'Paid' },
      pending: { class: 'bg-orange-100 text-orange-700', text: 'Pending' },
      outstanding: { class: 'bg-red-100 text-red-700', text: 'Outstanding' }
    };
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mt-4 text-sm text-gray-600">
        ...
        {localFilters.month && ` for ${localFilters.month}`}
