...
const MiscRow = ({ item }) => {
    return (
        <tr className="hover:bg-gray-50 dark:hover:bg-gray-700 border-b dark:border-gray-700">
            <td className="px-4 py-3">{item.other_activity_code}</td>
            <td className="px-4 py-3">{item.name}</td>
            <td className="px-4 py-3 text-center">IDR {formatRupiah(item.price)}</td>
        </tr>
    );
};

const MiscFilters = ({ filter }) => {
            ...
            <div className="min-h-screen">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">
                        Manage Miscellaneous Items
                    </h1>
                    <MiscFilters filter={filters} />
                </div>
