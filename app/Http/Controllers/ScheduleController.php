...
class ScheduleController extends Controller
{
    ...
    {
        $pending = Booking::with(['user','bookingDetail.package', 'bookingPayment' => function ($query) {
                $query->withoutGlobalScope('paid');
            }])->where('status', 'pending wise')->where('payment_method', 'pay later')->whereHas('bookingPayment', function ($query) {
            $query->withoutGlobalScope('paid')->where('is_paid', '0');
            ...
            return [
                ...
                'package_name' => $booking->bookingDetail[0]->package ? $booking->bookingDetail[0]->package->name : 'Custom Package',
                'package_url' => $booking->bookingDetail[0]->package ? "https://javavolcano-touroperator.com/" . $booking->bookingDetail[0]->package->new_slug : null,
                'total_pax' => $booking->total_pax,
                'payment_method' => 'Bank Transfer',
                'payment_proof' => $booking->bookingPayment->first() ? 'https://legacy.javavolcano-touroperator.com/' . $booking->bookingPayment->first()->proof_file : null,
            ];
        });
        return Inertia::render('Schedule/PendingBooking', ['data' => $pending]);
    }
        ...
        $package = Package::with([
            ...
            if (in_array(2, $destinationIds)) {
                $result['package']['before_you_go']['mandatory_requirements_for_ijen_trekking'] = [
                    "<b>Medical Check-up:</b>Ijen authorities require a doctor's health certificate. This check-up (costing IDR 35,000-50,000) will be conveniently arranged at your hotel in Bondowoso the evening before your trek. You only need your passport.",
                    "<b>Printed Passport Copy:</b>A clear, printed copy of your passport is essential for permit processing at the Ijen site."
                ];
            }
