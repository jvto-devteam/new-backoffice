              ...
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th 
                    scope="col" 
                    className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
                    onClick={() => handleSort('travel_date_start')}
                  >
                    <div className="flex items-center">
                      Tanggal Travel
                      {sortConfig.key === 'travel_date_start' && (
                        <span className="ml-1">
                          {sortConfig.direction === 'asc' ? '↑' : '↓'}
                        </span>
                      )}
                ...
                {currentItems.length > 0 ? (
                  currentItems.map((invoice) => (
                    <tr key={invoice.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{formatDate(invoice.inv_date)}</div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{invoice.customer}</div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{formatDate(invoice.travel_date_start)}</div>
                        <div className="text-xs text-gray-500">{invoice.duration} hari</div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-sm text-gray-900 max-w-xs truncate" title={invoice.package_name}>
                          {invoice.package_name || '-'}
                        </div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{invoice.total_pax}</div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
